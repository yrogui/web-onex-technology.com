...
function SectorCard({ Icon, label }: { Icon: FC<LucideProps>; label: string }) {
  return (
    <div className="relative w-[140px] h-[140px] md:w-[180px] md:h-[180px] bg-paper dark:bg-charcoal/40 border border-smoke/40 dark:border-charcoal rounded-[14px] shrink-0 flex flex-col items-center justify-end pb-4 md:pb-5">
      {/* Icon center anchored at 35% of card height */}
      <div className="absolute top-[35%] left-1/2 -translate-x-1/2 -translate-y-1/2">
        <Icon strokeWidth={1.5} className="w-[72px] h-[72px] md:w-[94px] md:h-[94px] text-charcoal dark:text-smoke" />
      </div>
      <span className="relative font-sans font-medium text-[11px] uppercase tracking-[0.1em] text-graphite dark:text-smoke text-center leading-tight px-3">
        {label}
      </span>
    </div>
  );
...
export function SectorMarquee() {
  ...
  return (
    <section className="py-16 bg-mist dark:bg-charcoal/30 border-t border-smoke/30 dark:border-charcoal" suppressHydrationWarning>
      <div className="max-w-[1400px] mx-auto px-8 lg:px-16 text-center mb-10">
        <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-graphite dark:text-accent">
          {t("eyebrow")}
        </p>
      </div>
